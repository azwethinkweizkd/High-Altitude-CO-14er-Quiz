...
function saveScore() {
	...
	userInputForm.addEventListener("submit", function (event) {
		...
		let sortHighScores = userHighScores.sort(
			(player1, player2) => player2.score - player1.score
		);

		if (sortHighScores.length > 10) {
			sortHighScores.pop();
		}

		localStorage.setItem("topUsers", JSON.stringify(sortHighScores));
		renderHighScores();
	});
}
...
function renderHighScores() {
	...
	for (var i = 0; i < userHighScores.length; i++) {
		...
		p.textContent =
			`${i + 1}. ` + userHighScores[i].user + " - " + userHighScores[i].score;
		p.setAttribute("data-index", i);
		p.setAttribute("style", "padding-top:1rem");
